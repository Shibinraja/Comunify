...
export function TabPanel({
  ...
  const [shouldRender, setShouldRender] = useState<boolean>(!props.hidden || render === 'always');
  ...
  useEffect(() => {
    if (!props.hidden || render === 'always') {
      ...
      ('requestIdleCallback' in window ? requestIdleCallback : setTimeout)(() => setShouldRender(true));
      ...
      // if there is no idle periods in the browser we can call a setTimeout callback in order to run them before the timeout elapses
      ('requestIdleCallback' in window ? requestIdleCallback : setTimeout)(() => setShouldRender(false));
    }
