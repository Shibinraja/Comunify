...
const SubscriptionCard:React.FC<Props> = ({subscriptionData}) => {
    ...
    return (
        <Fragment>
            {subscriptionData?.map((subscriptionPackages: SubscriptionPackages) => (
                <div key={subscriptionPackages?.id} className="mt-1.87  flex flex-col ">
                    <div className="border-gradient-rounded px-8 py-5 bg-white rounded-0.9 ">
                        <h5 className="flex items-center">
                            <span className="price font-Poppins font-semibold text-renewalPrice leading-3.1">{subscriptionPackages?.cost}</span>
                        ...
                        <div className="mt-2 ">
                            {subscriptionPackages?.features?.map((featuresData: string, index: number) => (
                                <div
