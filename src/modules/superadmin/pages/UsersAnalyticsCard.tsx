      ...
      <div className="flex gap-2.28">
        {fetchLoader ? (
          Array.from({ length: 3 }, (_, i) => i + 1).map((type: number) => (
            <Fragment key={type}>
              <div className={`flex  items-center justify-center rounded-0.9 w-full h-8.34 px-9 ${type === 1 ? 'bg-member1' : type === 2 ? ' bg-member2' : ' bg-member3'}`}>
          ...
          analyticsData.map((analytics) => (
            <div className={`flex  items-center justify-between rounded-0.9 w-full h-8.34 px-9 ${analytics.title.toLocaleLowerCase().trim() === 'total users' ? 'bg-member1' : analytics.title.toLocaleLowerCase().trim() === 'total active' ? ' bg-member2' : ' bg-member3'}`} key={Math.random()}>
              ...
              <div className=" w-[63px] h-[63px] bg-[#FBFCFD] rounded-full flex justify-center items-center">
